...
int main(int argc, char *argv[])
 {
  ...
  int i;
  WbDeviceTag ds[3];
  char ds_names[3][13]={"ds_left","ds_right","ds_left_1"};
  for (i=0;i<=2;i++)
  {
  ...
  {
   //check_keyboard();
   printf("%lf",wb_distance_sensor_get_value(ds[0]));
    printf("%lf",wb_distance_sensor_get_value(ds[1]));
    printf("%lf",wb_distance_sensor_get_value(ds[2]));
    if(autopilot) run_autopilot();
       if (counter > 0)
       {
       printf("1\n");
          counter--;
...
//	wb_robot_step(500);
        else 
        {
          ...
          for (i = 0; i <= 2; i++)
	{
              ds_values[i] = wb_distance_sensor_get_value(ds[i]);//得到距离值
	}  
           if (ds_values[0] >80 || ds_values[1] > 80||ds_values[2] > 80)
	{
	printf("4\n");
    	  counter = 10;
  	  flag=true;				
