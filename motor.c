...
//	run_autopilot();//自动驾驶
	while(wb_robot_step(TIME_STEP) != -1)
	{
		...
		if(avoid_obstacle_counter>0)
		{
			...
			autopilot = false;//out of autopolit
			speeds[LEFT] += 0.5*MAX_SPEED;//turn right
			speeds[RIGHT] -= 0.5*MAX_SPEED;//
			robot_set_speed(speeds[LEFT], speeds[RIGHT]);//set speed
		}
		else
		{
				for (q = 0; q < 2; q++)
				...
				if(ds_values[0]<500.0||ds_values[1]<500.0)//距离判断
				{
